...
from Property_Extractors import ExtractProperties
from RandomForest import RandomForest
from GLOBAL_PATHS import OUTPUT_PATH
...
class ImageApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Retinal Image Analysis")
    ...
    def open_image(self):
        ...
        if file_path:
            ...
            parent_root = os.path.abspath(os.path.join(current_dir, os.pardir))
            test_results_path = os.path.join(parent_root,'saveroot', 'test_results', filename)
    ...
    def show_properties(self):
        properties = [
            ...
            ("Artery Density:", f"{self.artery_density:.2f}"),
            ("Capillary Density:", f"{self.capillary_density:.2f}"),
            ("Large Vessel Density:", f"{self.LV_density:.2f}"),
            ("Vein Density:", f"{self.vein_density:.2f}"),
    ...
    def diagnose(self):
        feature_list = ['Age', 'FAZ_circularity', 'Art_density', 'Cap_density','LV_density', 'Vein_density', 'Artery_diameter_index',
       ...
       # RF = joblib.load('random_forest_model.pkl')
        ...
        age = self.get_age_by_id('..\\RandomForest\\Text_labels_and_Numerical_Data_no_surface_areas_csv.csv', self.filename.split('.')[0])
        ...
        prediction = RandomForest.GenerateClassificationResult(
            [age, self.FAZ_circularity, self.artery_density, self.capillary_density, self.LV_density,
